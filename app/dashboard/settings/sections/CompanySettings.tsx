  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(`Failed to save: ${error.message || error.hint || 'Unknown error. Check console for details.'}`)
      } else {
        // ✅ Check if company profile is now complete
        if (formData.abn && formData.streetAddress) {
          await supabase
      ...
      <SettingsCard title="">
        ...
        <div className="mb-4">
          ...
          <AddressFields
            ...
            state={formData.state}
            postcode={formData.postcode}
            onStreetAddressChange={(value) => updateField('streetAddress', value)}
            onSuburbChange={(value) => updateField('suburb', value)}
